...
def write_lammps_data_file(batch, filename, job_details, phi0_flag=True, style='OPLS', single_mol=False):
    smiles=job_details.to_parameterize
    if style == 'compass':
        params = {}
        params['bond'] = ['b0', 'k_b2', 'k_b3', 'k_b4']
        params['angle'] = ['theta0', 'k_a2', 'k_a3', 'k_a4', 'k_bb', 'k_ba0', 'k_ba1']
        params['dihedral'] = ['k1', 'k2', 'k3', 'phi1', 'phi2', 'phi3', 
            'k_mt1', 'k_mt2', 'k_mt3', 'k_bct10', 'k_bct20', 'k_bct30', 'k_bct11', 'k_bct21', 'k_bct31']
        params['improper'] = ['k2']
...
#             new_types = new_types.tolist()
            ...
            new_types = batch.get_data('angle', 'type').unique(dim=0, return_inverse=True)[1].tolist()*2
#             import ipdb
#             ipdb.set_trace()
            ...
            bond_indices = torch.tensor(
                learned_params['angle']['bond'].values.tolist()).view(-1,2).to(torch.long)
    ...
    if top in batch._data.keys():
        ...
        if len(df) != 0:
            ...
            learned_params[top] = df            
            bond_indices = torch.tensor(
                learned_params['dihedral']['bond'].values.tolist()).view(-1,3).to(torch.long)
            b0 = batch.get_data('bond', 'b0').view(-1)
            b0 = b0[bond_indices]
            learned_params[top]['b0'] = b0.tolist()
            angle_indices = torch.tensor(
                learned_params['dihedral']['angle'].values.tolist()).view(-1,2).to(torch.long)
    ...
    if top in batch._data.keys():
        ...
        if len(df) != 0:
            ...
            learned_params[top] = df
            angle_indices = torch.tensor(
                learned_params['improper']['angle'].values.tolist()).view(-1,3).to(torch.long)
            theta0 = batch.get_data('angle', 'theta0').view(-1)
            theta0 = theta0[angle_indices]
            learned_params[top]['theta0'] = theta0.tolist()
    ...
    tab = " "
    newline = "\n"
    f.write('LAMMPS data file generated by AuTopology for {}.'.format(smiles))
    f.write(newline)
    f.write(newline)
    ...
    for top in ['bond', 'angle', 'dihedral', 'improper']:
        if top in learned_params.keys(): #batch._data.keys():
            ...
            if learned_params[top].shape[0] == 0:
                continue
            f.write(str(learned_params[top].shape[0]) + tab + "{} types".format(top) + newline)
    f.write(newline)
    multiplier = 100
    if single_mol:
        f.write(format(-50, '<+13.5e') + tab + format(50, '<+13.5e') + tab+ "xlo xhi" + newline)
        f.write(format(-50, '<+13.5e') + tab + format(50, '<+13.5e') + tab+ "ylo yhi" + newline)
        f.write(format(-50, '<+13.5e') + tab + format(50, '<+13.5e') + tab+ "zlo zhi" + newline)
    else:
        f.write(format(xyz[:,0].min().item()*multiplier, '<+13.5e') + tab + format(
            xyz[:,0].max().item()*multiplier, '<+13.5e') + tab+ "xlo xhi" + newline)
        f.write(format(xyz[:,1].min().item()*multiplier, '<+13.5e') + tab + format(
            xyz[:,1].max().item()*multiplier, '<+13.5e') + tab+ "ylo yhi" + newline)
        f.write(format(xyz[:,2].min().item()*multiplier, '<+13.5e') + tab + format(
            xyz[:,2].max().item()*multiplier, '<+13.5e') + tab+ "zlo zhi" + newline)
    f.write(newline)
    ...
    for i in range(learned_params['node'].shape[0]):
        ...
        line += format(learned_params['node']['mass'].iloc[i], '<+13.5e') + tab
        f.write(line)
        f.write(newline)
    f.write(newline)
    top = 'pair'
    if top in batch._data.keys():
        ...
        for i in range(learned_params['node'].shape[0]):
            line = ''
            line += str(int(learned_params['node']['new_type'].iloc[i])) + tab
            line += format(learned_params['node']['epsilon'].iloc[i], '<+13.5e') + tab
            line += format(learned_params['node']['sigma'].iloc[i], '<+13.5e') + tab
            f.write(line)
            f.write(newline)
        f.write(newline)
    top = 'bond'
    ...
    if top in learned_params.keys() and learned_params[top].shape[0] != 0:
        ...
        for i in range(learned_params[top].shape[0]):
            ...
            for param in params[top]:
                line += format(learned_params[top][param].iloc[i], '<+13.5e') + tab
            f.write(line)
            f.write(newline)     
        f.write(newline)
    top = 'angle'
    ...
    if top in learned_params.keys() and learned_params[top].shape[0] != 0:
        ...
        for i in range(learned_params[top].shape[0]):
            line = ''
            line += str(int(learned_params[top]['new_type'].iloc[i])) + tab
            line += format(learned_params[top]['k2'].iloc[i], '<+13.5e')  + tab
            line += format(0.0, '<+13.5e')  + tab
            f.write(line)
            f.write(newline)
        f.write(newline)
    if style == 'compass':     
        top = 'angle'      
        if top in batch._data.keys() and top in learned_params.keys() and learned_params[top].shape[0] != 0:
            f.write("BondBond Coeffs" + newline)
            f.write(newline)
            for i in range(learned_params[top].shape[0]):
                line = ''
                line += str(int(learned_params[top]['new_type'].iloc[i])) + tab
                line += format(learned_params[top]['k_bb'].iloc[i], '<+13.5e')  + tab
                line += format(learned_params[top]['b0'].iloc[i][0], '<+13.5e')  + tab
                line += format(learned_params[top]['b0'].iloc[i][1], '<+13.5e')  + tab
                f.write(line)
                f.write(newline)     
            f.write(newline)
            ...
            for i in range(learned_params[top].shape[0]):
                line = ''
                line += str(int(learned_params[top]['new_type'].iloc[i])) + tab
                line += format(learned_params[top]['k_ba0'].iloc[i], '<+13.5e')  + tab
                line += format(learned_params[top]['k_ba1'].iloc[i], '<+13.5e')  + tab
                line += format(learned_params[top]['b0'].iloc[i][0], '<+13.5e')  + tab
                line += format(learned_params[top]['b0'].iloc[i][1], '<+13.5e')  + tab
                f.write(line)
                f.write(newline)     
        ...
        if top in batch._data.keys() and top in learned_params.keys() and learned_params[top].shape[0] != 0:
            ...
            for i in range(learned_params[top].shape[0]):
                line = ''
                line += str(int(learned_params[top]['new_type'].iloc[i])) + tab
                line += format(learned_params[top]['k_mt1'].iloc[i], '<+13.5e')  + tab
                line += format(learned_params[top]['k_mt2'].iloc[i], '<+13.5e')  + tab
                line += format(learned_params[top]['k_mt3'].iloc[i], '<+13.5e')  + tab
                line += format(learned_params[top]['b0'].iloc[i][1], '<+13.5e')  + tab
                f.write(line)
                f.write(newline)
            f.write(newline)
            ...
            for i in range(learned_params[top].shape[0]):
                line = ''
                line += str(int(learned_params[top]['new_type'].iloc[i])) + tab
                line += format(learned_params[top]['k_bct10'].iloc[i], '<+13.5e')  + tab
                line += format(learned_params[top]['k_bct20'].iloc[i], '<+13.5e')  + tab
                line += format(learned_params[top]['k_bct30'].iloc[i], '<+13.5e')  + tab
                line += format(learned_params[top]['k_bct11'].iloc[i], '<+13.5e')  + tab
                line += format(learned_params[top]['k_bct21'].iloc[i], '<+13.5e')  + tab
                line += format(learned_params[top]['k_bct31'].iloc[i], '<+13.5e')  + tab
                line += format(learned_params[top]['b0'].iloc[i][0], '<+13.5e')  + tab
                line += format(learned_params[top]['b0'].iloc[i][2], '<+13.5e')  + tab
                f.write(line)
                f.write(newline)
            f.write(newline)
            ...
            for i in range(learned_params[top].shape[0]):
                line = ''
                line += str(int(learned_params[top]['new_type'].iloc[i])) + tab
                line += format(learned_params[top]['k_at10'].iloc[i], '<+13.5e')  + tab
                line += format(learned_params[top]['k_at20'].iloc[i], '<+13.5e')  + tab
                line += format(learned_params[top]['k_at30'].iloc[i], '<+13.5e')  + tab
                line += format(learned_params[top]['k_at11'].iloc[i], '<+13.5e')  + tab
                line += format(learned_params[top]['k_at21'].iloc[i], '<+13.5e')  + tab
                line += format(learned_params[top]['k_at31'].iloc[i], '<+13.5e')  + tab
                line += format(learned_params[top]['theta0'].iloc[i][0]*180/3.1415926535, '<+13.5e')  + tab
                line += format(learned_params[top]['theta0'].iloc[i][1]*180/3.1415926535, '<+13.5e')  + tab
                f.write(line)
                f.write(newline)
            f.write(newline)
            ...
            for i in range(learned_params[top].shape[0]):
                line = ''
                line += str(int(learned_params[top]['new_type'].iloc[i])) + tab
                line += format(learned_params[top]['k_aat'].iloc[i], '<+13.5e')  + tab
                line += format(learned_params[top]['theta0'].iloc[i][0]*180/3.1415926535, '<+13.5e')  + tab
                line += format(learned_params[top]['theta0'].iloc[i][1]*180/3.1415926535, '<+13.5e')  + tab
                f.write(line)
                f.write(newline)
            ...
            for i in range(learned_params[top].shape[0]):
                line = ''
                line += str(int(learned_params[top]['new_type'].iloc[i])) + tab
                line += format(learned_params[top]['k_b1b3'].iloc[i], '<+13.5e')  + tab
                line += format(learned_params[top]['b0'].iloc[i][0], '<+13.5e')  + tab
                line += format(learned_params[top]['b0'].iloc[i][2], '<+13.5e')  + tab
                f.write(line)
                f.write(newline)
        ...
        if top in batch._data.keys() and top in learned_params.keys() and learned_params[top].shape[0] != 0:
            ...
            for i in range(learned_params[top].shape[0]):
                line = ''
                line += str(int(learned_params[top]['new_type'].iloc[i])) + tab
                line += format(learned_params[top]['k_aa1'].iloc[i], '<+13.5e')  + tab
                line += format(learned_params[top]['k_aa2'].iloc[i], '<+13.5e')  + tab
                line += format(learned_params[top]['k_aa3'].iloc[i], '<+13.5e')  + tab
                line += format(learned_params[top]['theta0'].iloc[i][0]*180/3.1415926535, '<+13.5e')  + tab
                line += format(learned_params[top]['theta0'].iloc[i][1]*180/3.1415926535, '<+13.5e')  + tab
                line += format(learned_params[top]['theta0'].iloc[i][2]*180/3.1415926535, '<+13.5e')  + tab
                f.write(line)
                f.write(newline)     
            f.write(newline)
    ...
    for i in range(len(types['node'])):
        ...
        line += format(charges[i], '<+13.5e') + tab
        line += format(xyz[i,0].tolist(), '<+13.5e') + tab
        line += format(xyz[i,1].tolist(), '<+13.5e') + tab
        line += format(xyz[i,2].tolist(), '<+13.5e') + tab
        f.write(line)
        f.write(newline)
    f.write(newline)
    for top in ['bond', 'angle', 'dihedral', 'improper']:
        if top in batch._data.keys():
            ...
            for i in range(len(types[top])):
                ...
                if top == 'angle':
                    ...
                    maskbatch = learned_params[top]['type'].apply(lambda x: x == batchtypes[i])
#                     import ipdb
#                     ipdb.set_trace()
                ...
                if top in ['dihedral', 'improper']:
                    ...
                    def compare(x, node_types):
                        return(x == node_types)
                    mask = learned_params[top]['node'].apply(lambda x: compare(x, node_types))
...
def write_traj(atomic_nums, infile, outfile):
    ...
    trajectory = np.concatenate(
        (
            np.array(z).reshape(trajectory.shape[0], trajectory.shape[1], 1),
            trajectory
        ), axis=2
    )
    file = open(outfile, 'w')
