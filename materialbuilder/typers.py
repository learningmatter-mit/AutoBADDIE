...
class CompassTyper(torch.nn.Module):
    def __init__(self, reference_geoms=None):
        ...
        self.patterns, self.variables, self.param_dicts = utils.read_ff_file(
            os.path.join(
                os.path.dirname(os.path.abspath(__file__)), "FFlibrary/compass.ff"
            )
        )
        self.params = {}
        ...
        for term in self.param_dicts.keys():
            self.params[term] = {
                var: torch.tensor(self.param_dicts[term][var].values.tolist())
    ...
    def get_types(self, graph, top):
        ...
        top_types = torch.cat(
            [top_types, torch.zeros(len(top_types), 1).to(torch.long)], dim=1
        )
